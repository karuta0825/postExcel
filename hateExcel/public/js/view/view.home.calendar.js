...
(function ($, cms) {
  ...
  _drawCalendar = function (diff) {
    ...
    // データ取得
    const tableMap = _getCalender(diff);
  ...
  _markCalendar = function (list_event) {
    // 一度リセット
    _.each(view.get('calendar__body').find('td'), (v, k) => {
      $(v).removeClass('is-event');
    });

    _.each(list_event, (v, k) => {
  ...
  _showError = function (list) {
    // 初期化
    _.each(view.get('event-input'), (val, key) => {
